...
namespace TallyJ4.Extensions
{
  public static class MiscExtensions
  {
    ...
    /// <summary>
    /// The rightmost letters in the string. If the string is shorter than length it is returned as is.
    /// </summary>
    /// <param name="input"></param>
    ...
    {
      return input.HasNoContent() ? "" : input.Length < length ? input : input.Substring(input.Length - length);
    }
    ...
    public static HtmlString PercentInSpan(this int num, int total, int decimals = 0, bool surroundWithParen = false,
      bool showZero = true)
    {
      return new HtmlString("<span class=pct>" + num.PercentOf(total, decimals, surroundWithParen, showZero) + "</span>");
    }
